...
- `relay_rules.tunnel_id` NULL means "not deployed anywhere" (not part of any node's config). `relay_rules.user_id` NULL means admin-managed (never quota-gated).
