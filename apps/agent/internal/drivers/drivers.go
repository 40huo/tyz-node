// Package drivers registers every GOST driver, verbatim from gost's own
// cmd/gost/register.go (matching core v0.6.0 / x v0.15.2). The full set is
// intentional: GOST's parsers default to connector types (e.g. "http" for
// forwarder nodes) and the auto handler probes http/socks handlers, so a
// curated subset would break on those implicit defaults.
package drivers
