...
| I11 | **协议裁剪：配置 payload 移除 `agent` 判别字段（2026-08-30，整体切换的彻底化）**：`RealmNodeConfig` 不再携带 `agent: "realm"`（shared 类型/zod schema/渲染器/agent 模型与 translate 同步移除）；server 的 GET /config 不再做非 realm 快照自愈守卫。切换已整体完成（§14 决策 D3/Q5），线上仅存在单一格式，判别字段属于过渡期产物 | 后果已向所有方明示：若某环境的 `node_configs` 仍残留 gost 旧格式行，新 agent 会将其解码为空服务集并应用（旧格式对 serde 不可区分），等于该节点静默断服直至下一次重算——处置即"重算掉旧行"（管理写入/每日 cron），不提供兼容路径 |
| I12 | **限速/限连落地（2026-08-30，替代"rule.limit INERT"的过渡状态）**：payload `RealmService` 增加 `limit { service_in, service_out, conn_in, conn_out, max_conns }`（由 rule.limit 的 `traffic.*` 与 `connection.service_limit` 渲染；`request.*` 与按 IP 细分不适用于裸 TCP 转发，丢弃）。agent 侧新增 `ratelimit.rs`（字节令牌桶 + Pacer 多桶取 min + poll 友好 Waiter + Paced 用户态包装）；splice 状态机的写侧按授权字节封顶切片长度、桶干涸时 Pending 停泊——零拷贝保留，仅节流节奏；TLS 用户态腿经 Paced 走同一批桶；`connection.service_limit` 在 accept 层拒绝超额连接 | `rule.limit` 从面板摆设变为真实执行；限速语义 = 双向分别计桶（service 级共享 + conn 级独立，取 min）；测试覆盖桶数学、Pacer 取 min、限速端到端节奏（128KB@256KB/s ≥0.4s）、连接上限拒绝 |
